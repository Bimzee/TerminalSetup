...
function Parse-GitAliases {
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match '\\$') {
            # Remove trailing backslash and whitespace, accumulate
...
$aliases = Parse-GitAliases -Content $aliasContent
...
# Step 5: Show installed aliases
Show-InstalledAliases

# Step 6: Install and configure posh-git (if not skipped)
if (-not $SkipPoshGit) {
    if (Install-PoshGit) {
